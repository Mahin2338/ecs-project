...
A containerized deployment of Umami (open-source web analytics) on AWS using ECS Fargate, with full CI/CD pipeline and infrastructure as code.
...
- **ECS Fargate**: Runs the containerized Umami application
...
The build process is specific to Umami - it uses separate build commands for different parts of the app instead of a single build command. This avoids issues with database connections during the build.
...
Everything is defined in Terraform and organized into modules. The VPC module creates the network foundation with public and private subnets. ECS runs the containers, RDS handles the database, and the ALB manages traffic.
...
I chose Fargate over EC2 because it's simpler - no servers to patch or maintain. It's more expensive than running your own EC2 instances, but for a learning project the tradeoff is worth it.
...
**NAT Gateway costs**: These cost about $32/month each just to exist, plus data transfer fees. I'm using one NAT Gateway instead of two (one per AZ) to save costs. In production you'd want two for true high availability.
...
The multi-stage Docker build was interesting too. It's not obvious at first why you'd want two stages, but when you see the size difference (almost 2gb vs 150 -300MB) it makes sense.
